...
export async function postNoSDKComment(options: PRCommentOptions): Promise<void> {
  ...
  const commentBody = `## 🔍 RudderStack Instrumentation Review

### ℹ️ No RudderStack SDK Detected

This PR does not contain any RudderStack SDK instrumentation.

If you're adding RudderStack for the first time, make sure to:
- Install the SDK via NPM: \`npm install @rudderstack/analytics-js\`
- Or add the CDN snippet to your HTML
