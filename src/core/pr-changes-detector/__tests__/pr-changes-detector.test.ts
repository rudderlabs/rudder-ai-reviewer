...
describe('PRChangesDetector', () => {
  ...
  it('should detect PR changes successfully', async () => {
    // Arrange - Mock GitHubClient methods
    ...
    // Act
    const detector = new PRChangesDetector(mockGitHubClient as any, mockPRContext);
  ...
  it('should propagate errors from GitHub client', async () => {
    ...
    // Act & Assert
    const detector = new PRChangesDetector(mockGitHubClient as any, mockPRContext);
    await expect(detector.detect()).rejects.toThrow('API error');
  });
});
